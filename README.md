...
已批准且启用的有效规则可自动复用：明确语义按问题类别匹配，未解析证据要求完整模板集及类型、组件等条件一致。历史混合证据拆分或匹配条件变化时需重新审批。离线修复可依据原始迁移快照恢复误标版本的 V1 规则，并重分类未人工编辑的待审批候选；修复前需停止服务、备份数据库并预览，已批准/驳回记录与原审批范围保持不变。
