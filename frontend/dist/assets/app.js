(function () {
  ...
  const api = {
    ...
    agentWorkflows: function () { return jsonRequest("/api/agent-workflows"); },
    agentWorkflowRuns: function () { return jsonRequest("/api/agent-workflow-runs"); },
    agentWorkflowRun: function (runId) { return jsonRequest("/api/agent-workflow-runs/" + encodeURIComponent(runId)); },
    createAgentWorkflow: function (definition) { const key = "agent-workflow-" + Date.now(); return jsonRequest("/api/agent-workflows", { method: "POST", headers: { "Idempotency-Key": key }, body: JSON.stringify(definition) }); },
    createAgentWorkflowRun: function (workflowId, payload) { const key = "agent-workflow-run-" + Date.now(); return jsonRequest("/api/agent-workflows/" + encodeURIComponent(workflowId) + "/runs", { method: "POST", headers: { "Idempotency-Key": key }, body: JSON.stringify(payload) }); },
    agentWorkflowAction: function (runId, action) { const key = "agent-workflow-control-" + Date.now(); return jsonRequest("/api/agent-workflow-runs/" + encodeURIComponent(runId) + "/actions/" + action, { method: "POST", headers: { "Idempotency-Key": key }, body: JSON.stringify({ idempotency_key: key }) }); },
    retryNode: function (runId, nodeId) { const key = "agent-workflow-node-" + Date.now(); return jsonRequest("/api/agent-workflow-runs/" + encodeURIComponent(runId) + "/nodes/" + encodeURIComponent(nodeId) + "/retry", { method: "POST", headers: { "Idempotency-Key": key }, body: JSON.stringify({ idempotency_key: key }) }); },
    ...
    runtimeTasks: function () { return jsonRequest("/api/runtime/tasks?page_size=100"); },
    ...
    observations: function () { return jsonRequest("/api/observability-v2/observations?limit=50"); },
    observationTimeline: function (id, query) { return jsonRequest("/api/observability-v2/observations/" + encodeURIComponent(id) + "/timeline" + (query || "")); },
    observabilitySpan: function (id) { return jsonRequest("/api/observability-v2/spans/" + encodeURIComponent(id)); },
    observabilityMetricsV2: function () { return jsonRequest("/api/observability-v2/metrics"); },
    createReplay: function (payload) { return jsonRequest("/api/observability-v2/replays", { method: "POST", headers: { "Idempotency-Key": "ui-" + Date.now() }, body: JSON.stringify(payload) }); },
    replay: function (id) { return jsonRequest("/api/observability-v2/replays/" + encodeURIComponent(id)); },
    replayDiff: function (id) { return jsonRequest("/api/observability-v2/replays/" + encodeURIComponent(id) + "/diff"); },
  ...
  function pathToView(path) {
    ...
    if (path === "/agent-workflows") return "agentWorkflows";
  ...
  function routeForView(view) {
    return {
      review: "/review", queue: "/queue", export: "/export",
      benchmarkCenter: "/benchmark-center", runtime: "/runtime", releaseReadiness: "/release-readiness", streaming: "/streaming", prompts: "/prompts", modelProfiles: "/model-profiles", agentRuns: "/agent-runs", agentWorkflows: "/agent-workflows",
  ...
  function AIObservabilityPage(props) {
    ...
    const [v2, setV2] = useState({ observations: [], timeline: [], metrics: null, selected: null, error: "" });
    const [stageFilter, setStageFilter] = useState("");
    const [replayMode, setReplayMode] = useState("historical");
    const [replayState, setReplayState] = useState(null);
    const [replayDiff, setReplayDiff] = useState(null);
    const [selectedSpan, setSelectedSpan] = useState(null);
    const estimatedCost = v2.metrics && v2.metrics.estimated_cost;
    const estimatedCostText = !estimatedCost ? "不可计算" : (
      estimatedCost.amount !== undefined
        ? estimatedCost.currency + " " + Number(estimatedCost.amount).toFixed(6)
        : Object.keys(estimatedCost.by_currency || {}).map(function (currency) { return currency + " " + Number(estimatedCost.by_currency[currency]).toFixed(6); }).join(" · ")
    );
    const stageMetrics = v2.metrics && v2.metrics.stages || {};
    const maxP95 = Math.max.apply(null, [0].concat(Object.keys(stageMetrics).map(function (stage) { return Number(stageMetrics[stage].p95_latency_ms || 0); })));
    function loadTimeline(observation, stage) {
      if (!observation) return Promise.resolve();
      const query = stage ? "?stage=" + encodeURIComponent(stage) : "";
      return api.observationTimeline(observation.observation_id, query).then(function (timeline) {
        setV2(function (current) { return Object.assign({}, current, { selected: observation, timeline: timeline.items || [], error: "" }); });
      });
    }
    function openSpan(spanId) {
      api.observabilitySpan(spanId).then(setSelectedSpan).catch(function (reason) {
        setV2(function (current) { return Object.assign({}, current, { error: reason.message }); });
      });
    }
    useEffect(function () {
      Promise.all([api.observations(), api.observabilityMetricsV2()]).then(function (values) {
        const observations = values[0].items || [], selected = observations[0] || null;
        setV2({ observations: observations, timeline: [], metrics: values[1], selected: selected, error: "" });
        return loadTimeline(selected, "");
      }).catch(function (reason) { setV2(function (current) { return Object.assign({}, current, { error: reason.message }); }); });
    }, [progress && progress.job_id]);
    function startReplay(traceId) {
      if (!traceId) return;
      if (!window.confirm(replayMode === "model" ? "重新调用原模型会产生真实调用，确认继续？" : "确认使用历史输出重新执行校验？")) return;
      setReplayDiff(null);
      api.createReplay({ source_trace_id: traceId, mode: replayMode, confirmed: true, operator: "local-operator" }).then(function (created) {
        setReplayState(created);
        function poll(remaining) {
          api.replay(created.replay_id).then(function (current) {
            setReplayState(current);
            if ((current.status === "pending" || current.status === "running") && remaining > 0) {
              window.setTimeout(function () { poll(remaining - 1); }, 500);
              return;
            }
            if (current.status === "completed") api.replayDiff(created.replay_id).then(setReplayDiff);
          }).catch(function (reason) { setReplayState({ status: "failed", error_message: reason.message }); });
        }
        poll(40);
      }).catch(function (reason) { setReplayState({ status: "failed", error_message: reason.message }); });
    }
    ...
    return h(React.Fragment, null,
      ...
      h("section", { className: "surface observability-v2" },
        h("div", { className: "surface-head" }, h("div", null, h("b", null, "分析链路时间线"), h("span", null, "Observation · Span · Replay")), h("div", { className: "obs-v2-metrics" }, h("span", null, "Span " + (v2.metrics && v2.metrics.span_count || 0)), h("span", null, "阶段最大 P95 " + maxP95 + " ms"), h("span", null, "Token 用量 " + (v2.metrics && v2.metrics.usage && (v2.metrics.usage.input_tokens + v2.metrics.usage.output_tokens) || 0)), h("span", null, "估算成本 " + estimatedCostText))),
        v2.error && h("div", { className: "empty-state error" }, v2.error),
        !v2.error && !v2.observations.length && h("div", { className: "empty-state" }, "暂无 Span 数据。旧版 Trace 仍可在 AI 调用追踪中查看。"),
        v2.observations.length > 0 && h("div", { className: "obs-v2-layout" },
          h("div", { className: "obs-v2-runs" }, v2.observations.map(function (item) { return h("button", { key: item.observation_id, className: v2.selected && v2.selected.observation_id === item.observation_id ? "active" : "", onClick: function () { loadTimeline(item, stageFilter); } }, h("b", null, item.job_id), h("small", null, item.status + " · " + timeText(item.updated_at))); })),
          h("div", { className: "obs-v2-timeline" },
            h("div", { className: "obs-v2-toolbar" }, h("select", { value: stageFilter, onChange: function (event) { const next = event.target.value; setStageFilter(next); loadTimeline(v2.selected, next); } }, h("option", { value: "" }, "全部阶段"), ["input", "normalize", "drain3", "aggregate", "rule_cache", "evidence", "prompt", "model", "parse", "schema", "evaluator", "candidate", "approval"].map(function (stage) { return h("option", { key: stage, value: stage }, stage); })), h("span", null, "失败阶段可通过状态颜色识别")),
            v2.timeline.map(function (span) { return h("article", { className: "obs-span " + span.status, key: span.span_id }, h("i"), h("div", null, h("b", null, span.stage + " · " + span.name), h("small", null, span.status + " · " + (span.duration_ms || (span.attributes || {}).latency_ms || 0) + " ms")), h("button", { className: "text-button", onClick: function () { openSpan(span.span_id); } }, "详情"), span.trace_id && h("button", { className: "text-button", onClick: function () { props.onOpenTrace(span.trace_id); } }, "Trace")); }),
            !v2.timeline.length && h("div", { className: "empty-state compact" }, "当前筛选没有 Span"))),
        h("div", { className: "replay-panel" }, h("div", null, h("b", null, "安全 Replay"), h("span", null, "历史结果重放不调用模型；重新调用原模型需要人工确认，且结果不会写入规则。")), h("select", { value: replayMode, onChange: function (event) { setReplayMode(event.target.value); } }, h("option", { value: "historical" }, "历史结果重放"), h("option", { value: "model" }, "重新调用原模型")), h("button", { className: "secondary-button", disabled: !(progress && progress.entities || []).some(function (item) { return item.trace_id; }), onClick: function () { const entity = (progress && progress.entities || []).find(function (item) { return item.trace_id; }); startReplay(entity && entity.trace_id); } }, "创建 Replay"), replayState && h("span", { className: "status-chip " + replayState.status }, replayState.error_message || replayState.status), replayDiff && h("div", { className: "replay-diff" }, h("b", null, replayDiff.changed ? "版本差异：有变化" : "版本差异：无变化"), h("span", null, "新增 " + (replayDiff.added || 0) + " · 删除 " + (replayDiff.removed || 0) + " · 修改 " + (replayDiff.modified || 0))))),
      ...
      h(Drawer, { title: "Span 详情", subtitle: selectedSpan && (selectedSpan.stage + " · " + selectedSpan.name), item: selectedSpan, onClose: function () { setSelectedSpan(null); } },
        selectedSpan && h("div", { className: "span-detail" },
          h("div", { className: "button-row" },
            selectedSpan.trace_id && h("button", { className: "secondary-button", onClick: function () { props.onOpenTrace(selectedSpan.trace_id); } }, "查看 Trace"),
            progress.prompt_id && h("button", { className: "secondary-button", onClick: function () { props.onOpenPrompt(progress.prompt_id); } }, "查看 Prompt"),
            progress.model_profile_id && h("button", { className: "secondary-button", onClick: props.onModels }, "查看模型 Profile"),
            h("button", { className: "secondary-button", onClick: props.onRules }, "查看规则")),
          h(CodeBlock, { value: { span_id: selectedSpan.span_id, observation_id: selectedSpan.observation_id, status: selectedSpan.status, duration_ms: selectedSpan.duration_ms, attributes: selectedSpan.attributes } }))));
  }
  ...
  function ModelProfilesPage(props) {
    ...
    const detail = h("div", { className: "surface" }, h("div", { className: "surface-head" }, h("b", null, "当前 Profile 配置"), h("span", null, draft.profile_id || "—")),
      h("div", { className: "profile-detail" },
        ...
        h("h3", null, "Token 成本（可选）"),
        h("div", { className: "budget-grid" },
          h("label", null, h("b", null, "每百万输入 Token"), h("input", { type: "number", min: "0", step: "0.000001", value: draft.input_price_per_million == null ? "" : draft.input_price_per_million, onChange: function (event) { setField("input_price_per_million", event.target.value === "" ? null : Number(event.target.value)); } })),
          h("label", null, h("b", null, "每百万输出 Token"), h("input", { type: "number", min: "0", step: "0.000001", value: draft.output_price_per_million == null ? "" : draft.output_price_per_million, onChange: function (event) { setField("output_price_per_million", event.target.value === "" ? null : Number(event.target.value)); } })),
          h("label", null, h("b", null, "币种"), h("input", { value: draft.pricing_currency || "", placeholder: "CNY", onChange: function (event) { setField("pricing_currency", event.target.value); } }))),
  ...
  function RuntimeCenterPage(props) {
    const data = props.data || {};
    const identity = data.identity || {};
    const readiness = data.readiness || {};
    const storage = data.storage || {};
    const retention = data.retention || {};
    const effective = retention.effective || retention.configured || {};
    const tasks = data.tasks && data.tasks.items || [];
    const audits = data.audits && data.audits.items || [];
    const [working, setWorking] = useState("");
    const [taskSort, setTaskSort] = useState("updated");
    const [retentionDays, setRetentionDays] = useState(String(effective.completed_days || 30));
    useEffect(function () { setRetentionDays(String(effective.completed_days || 30)); }, [effective.completed_days]);
    async function preview() {
      setWorking("preview");
      try { await props.onPreview(); }
      finally { setWorking(""); }
    }
    async function execute() {
      if (!window.confirm("确认按当前 Retention 策略清理已过期的文件产物？此操作不会删除数据库记录或原始日志。")) return;
      setWorking("execute");
      try { await props.onExecute(); }
      finally { setWorking(""); }
    }
    async function savePolicy() {
      ...
      try {
        await props.onSavePolicy({
          expected_version: Number(retention.policy && retention.policy.version || 0),
          policy: {
            enabled: !!effective.enabled,
            completed_days: Number(retentionDays),
            trace_days: Number(effective.trace_days || 30),
            cache_days: Number(effective.cache_days || 14),
          },
        });
      } finally { setWorking(""); }
    }
    function copyDiagnostic() {
      const value = JSON.stringify({ page: "runtime", api_base: currentApiBase(), readiness: readiness.status || "unknown", quota: storage.level || "unknown", failure: data.failure || null, failure_code: data.failureCode || null }, null, 2);
      if (navigator.clipboard) navigator.clipboard.writeText(value);
    }
    if (data.loading) return h("div", { className: "runtime-page" }, h("div", { className: "runtime-state" }, h("b", null, "正在读取运行状态"), h("span", null, "正在汇总任务、存储、保留策略和审计记录…")));
    if (data.failure) return h("div", { className: "runtime-page" }, h("div", { className: "runtime-state error" }, h("b", null, data.failureCode === "runtime_identity_required" ? "缺少外部身份上下文" : "运行中心数据加载失败"), h("span", null, data.failure), h("div", null, h("button", { className: "secondary-button", onClick: props.onRefresh }, "重试"), h("button", { className: "text-button", onClick: copyDiagnostic }, "复制诊断信息"))));
    const taskStatus = tasks.filter(function (item) { return ["running", "queued", "pending"].includes(item.status); }).length;
    const sortedTasks = tasks.slice().sort(function (left, right) {
      if (taskSort === "status") return String(left.status).localeCompare(String(right.status)) || String(right.updated_at).localeCompare(String(left.updated_at));
      if (taskSort === "kind") return String(left.kind).localeCompare(String(right.kind)) || String(right.updated_at).localeCompare(String(left.updated_at));
      return String(right.updated_at).localeCompare(String(left.updated_at));
    });
    return h("div", { className: "runtime-page" },
      h("section", { className: "runtime-hero" },
        h("div", null, h("span", { className: "eyebrow" }, "PRODUCTION RUNTIME"), h("h2", null, "运行中心"), h("p", null, "查看任务、存储配额、Retention 执行和审计记录；身份由外部 PACAS / RBAC 代理传入。")),
        h("div", { className: "runtime-identity" }, h("span", { className: identity.authenticated ? "status-chip completed" : "status-chip" }, identity.authenticated ? "身份已识别" : "身份未启用"), h("b", null, identity.actor || "未提供主体"), h("small", null, identity.source === "loopback" ? "本机开发绕过" : (identity.roles || []).join(" · ") || "由外部身份边界管理"))),
      h("section", { className: "runtime-metrics" },
        [[readiness.ready ? "Ready" : "Not ready", "就绪状态", readiness.ready ? "ok" : "risk"], [String(tasks.length), "已登记任务", ""], [String(taskStatus), "运行 / 排队", taskStatus ? "warning" : ""], [runtimeBytes(storage.total_bytes), "存储用量", storage.level || ""], [String(audits.length), "审计记录", ""]].map(function (item) { return h("article", { key: item[1], className: item[2] || "" }, h("b", null, item[0]), h("span", null, item[1])); })),
      h("section", { className: "surface runtime-task-table" }, h("div", { className: "surface-head" }, h("div", null, h("b", null, "任务中心"), h("span", null, "跨特征识别、流式处理、评测和回放的统一只读视图")), h("div", { className: "runtime-toolbar" }, h("label", null, h("span", null, "任务排序"), h("select", { value: taskSort, onChange: function (event) { setTaskSort(event.target.value); } }, h("option", { value: "updated" }, "最近更新"), h("option", { value: "status" }, "状态"), h("option", { value: "kind" }, "类型"))), h("button", { className: "text-button", onClick: props.onRefresh }, "刷新"))),
        !tasks.length && h("div", { className: "empty-state compact" }, "当前没有已登记任务"),
        sortedTasks.slice(0, 12).map(function (task) { return h("article", { key: task.kind + task.task_id }, h("div", null, h("b", null, task.kind), h("small", null, task.task_id), h("small", null, "阶段：" + task.stage)), h("span", { className: "status-chip " + task.status }, task.status), h("span", null, task.progress == null ? "—" : Math.round(Number(task.progress) * 100) + "%"), h("small", null, (task.actions || []).join(" · ") || "仅查看"), h("small", null, timeText(task.updated_at))); })),
      h("section", { className: "runtime-grid" },
        h("article", { className: "surface runtime-retention" }, h("div", { className: "surface-head" }, h("div", null, h("b", null, "Retention / Rotation"), h("span", null, "仅清理受控文件产物；预览不删除文件")), h("span", { className: effective.enabled ? "status-chip completed" : "status-chip" }, effective.enabled ? "已启用" : "未启用")),
          h("div", { className: "runtime-retention-body" }, h("label", null, h("span", null, "已完成任务保留天数"), h("input", { type: "number", min: 1, max: 3650, value: retentionDays, onChange: function (event) { setRetentionDays(event.target.value); } })), h("p", null, "Trace " + (effective.trace_days || "—") + " 天 · Cache " + (effective.cache_days || "—") + " 天 · 策略版本 " + (retention.policy && retention.policy.version || 0)), h("div", { className: "runtime-actions" }, h("button", { className: "secondary-button", disabled: !!working, onClick: savePolicy }, working === "policy" ? "保存中…" : "保存策略"), h("button", { className: "secondary-button", disabled: !!working, onClick: preview }, working === "preview" ? "计算中…" : "预览清理"), h("button", { className: "danger-button", disabled: !!working || !effective.enabled, onClick: execute }, working === "execute" ? "清理中…" : "执行清理")))),
        h("article", { className: "surface runtime-storage" },
          h("div", { className: "surface-head" },
            h("div", null, h("b", null, "存储用量"), h("span", null, "状态文件与输出文件；不包含原始日志内容")),
            h("span", { className: "status-chip " + (storage.level || "") }, storage.level || "unknown")),
          h("div", { className: "runtime-storage-body" },
            h("b", null, runtimeBytes(storage.total_bytes)),
            h("div", { className: "runtime-capacity" }, h("i", { style: { width: Math.min(100, Math.round(Number(storage.total_bytes || 0) / Math.max(1, Number(storage.hard_limit_bytes || 1)) * 100)) + "%" } })),
            h("span", null, "预警 " + runtimeBytes(storage.soft_limit_bytes) + " · 硬限制 " + runtimeBytes(storage.hard_limit_bytes)),
            h("small", null, "state " + runtimeBytes(storage.state_bytes) + " / " + (storage.state_files || 0) + " 个文件；output " + runtimeBytes(storage.output_bytes) + " / " + (storage.output_files || 0) + " 个文件"))),
      ),
  ...
  function AgentWorkflowsPage(props) {
    const workflows = props.data.workflows || [], runs = props.data.runs || [], selected = props.data.selected;
    const roles = [
      { role_id: "evidence_specialist", name: "证据专家", kind: "确定性工具", description: "读取聚合脱敏 Evidence" },
      { role_id: "rule_specialist", name: "规则专家", kind: "确定性工具", description: "查询批准规则与知识资产" },
      { role_id: "feature_specialist", name: "特征专家", kind: "模型推断", description: "校验并登记待审批资产" },
    ];
    const defaultNodes = [{ node_id: "evidence", role_id: "evidence_specialist", depends_on: [] }, { node_id: "rules", role_id: "rule_specialist", depends_on: [] }, { node_id: "feature", role_id: "feature_specialist", depends_on: ["evidence", "rules"] }];
    const [definition, setDefinition] = useState({ name: "节点风险特征协作", description: "固定角色并行收集证据后生成待审批候选", nodes: defaultNodes, max_concurrency: 2, max_tool_calls: 20, timeout_seconds: 300, max_attempts: 2 });
    const [runDraft, setRunDraft] = useState({ workflow_id: "", source_job_id: "", entity_id: "", model_profile_id: "", prompt_id: "agent_plan_v1" });
    function createWorkflow() { return props.onCreateWorkflow({ schema_version: "1.0", name: definition.name, description: definition.description, nodes: definition.nodes, budget: { max_nodes: definition.nodes.length, max_concurrency: definition.max_concurrency, max_tool_calls: definition.max_tool_calls, timeout_seconds: definition.timeout_seconds }, retry_policy: { max_attempts: definition.max_attempts } }); }
    function createRun() { return props.onCreateRun(runDraft.workflow_id, runDraft); }
    function nodeId(roleId) { return { evidence_specialist: "evidence", rule_specialist: "rules", feature_specialist: "feature" }[roleId]; }
    function toggleRole(roleId) { const id = nodeId(roleId), exists = definition.nodes.some(function (node) { return node.node_id === id; }); let nodes = exists ? definition.nodes.filter(function (node) { return node.node_id !== id; }).map(function (node) { return Object.assign({}, node, { depends_on: node.depends_on.filter(function (dependency) { return dependency !== id; }) }); }) : definition.nodes.concat([{ node_id: id, role_id: roleId, depends_on: [] }]); setDefinition(Object.assign({}, definition, { nodes: nodes })); }
    function toggleDependency(targetId, dependencyId) { const nodes = definition.nodes.map(function (node) { if (node.node_id !== targetId) return node; const exists = node.depends_on.includes(dependencyId); return Object.assign({}, node, { depends_on: exists ? node.depends_on.filter(function (item) { return item !== dependencyId; }) : node.depends_on.concat([dependencyId]) }); }); setDefinition(Object.assign({}, definition, { nodes: nodes })); }
    function actionLabel(action) { return { pause: "暂停", resume: "继续", cancel: "取消", retry: "重试", replay: "只读回放" }[action]; }
    function actions(status) { const value = []; if (["queued", "running"].includes(status)) value.push("pause"); if (status === "paused") value.push("resume"); if (["queued", "running", "paused", "failed", "awaiting_human"].includes(status)) value.push("cancel"); if (["failed", "cancelled"].includes(status)) value.push("retry"); value.push("replay"); return value; }
    return h("section", { className: "agent-workflows-page" },
      h("div", { className: "agent-runs-hero" }, h("div", null, h("span", { className: "eyebrow" }, "CONTROLLED ROLE DAG"), h("h2", null, "工作流编排"), h("p", null, "固定角色、显式依赖、并行调度与人工 Gate。不能动态创建 Agent、修改运行中 DAG 或绕过审批。")), h("button", { className: "secondary-button", onClick: props.onRefresh }, "刷新")),
      h("div", { className: "workflow-legend" }, h("span", null, "模型推断"), h("span", null, "确定性工具"), h("span", null, "人工 Gate"), h("span", null, "待审批资产")),
      h("div", { className: "workflow-grid" },
        h("section", { className: "surface workflow-designer" }, h("div", { className: "section-title" }, h("h3", null, "Workflow Designer"), h("span", null, "固定角色")),
          h("label", null, "名称", h("input", { value: definition.name, onChange: function (event) { setDefinition(Object.assign({}, definition, { name: event.target.value })); } })),
          h("div", { className: "workflow-role-cards" }, roles.map(function (role) { const node = definition.nodes.find(function (item) { return item.role_id === role.role_id; }), candidates = definition.nodes.filter(function (item) { return node && item.node_id !== node.node_id; }); return h("article", { key: role.role_id, className: "workflow-role-card " + (node ? "enabled" : "disabled") }, h("label", { className: "workflow-role-toggle" }, h("input", { type: "checkbox", checked: Boolean(node), onChange: function () { toggleRole(role.role_id); } }), h("b", null, role.name)), h("span", null, role.kind), h("p", null, role.description), node && h("div", { className: "workflow-dependencies" }, h("small", null, "依赖节点"), candidates.length ? candidates.map(function (candidate) { return h("label", { key: candidate.node_id }, h("input", { type: "checkbox", checked: node.depends_on.includes(candidate.node_id), onChange: function () { toggleDependency(node.node_id, candidate.node_id); } }), candidate.node_id); }) : h("em", null, "无可选依赖"))); })),
          h("div", { className: "workflow-budget-fields" }, h("label", null, "并发上限", h("input", { type: "number", value: definition.max_concurrency, onChange: function (event) { setDefinition(Object.assign({}, definition, { max_concurrency: Number(event.target.value) })); } })), h("label", null, "ToolCall 预算", h("input", { type: "number", value: definition.max_tool_calls, onChange: function (event) { setDefinition(Object.assign({}, definition, { max_tool_calls: Number(event.target.value) })); } })), h("label", null, "重试次数", h("input", { type: "number", value: definition.max_attempts, onChange: function (event) { setDefinition(Object.assign({}, definition, { max_attempts: Number(event.target.value) })); } }))),
          h("button", { className: "primary-button", disabled: !definition.nodes.length, onClick: createWorkflow }, "保存工作流")),
        h("section", { className: "surface workflow-run-create" }, h("div", { className: "section-title" }, h("h3", null, "启动工作流"), h("span", null, workflows.length + " 个定义")),
          h("label", null, "工作流", h("select", { value: runDraft.workflow_id, onChange: function (event) { setRunDraft(Object.assign({}, runDraft, { workflow_id: event.target.value })); } }, h("option", { value: "" }, "请选择"), workflows.map(function (workflow) { return h("option", { key: workflow.workflow_id, value: workflow.workflow_id }, workflow.name); }))),
          [["source_job_id", "来源任务"], ["entity_id", "风险实体"]].map(function (field) { return h("label", { key: field[0] }, field[1], h("input", { value: runDraft[field[0]], onChange: function (event) { setRunDraft(Object.assign({}, runDraft, { [field[0]]: event.target.value })); } })); }),
          h("label", null, "模型 Profile", h("select", { value: runDraft.model_profile_id, onChange: function (event) { setRunDraft(Object.assign({}, runDraft, { model_profile_id: event.target.value })); } }, h("option", { value: "" }, "请选择"), (props.profiles || []).map(function (profile) { return h("option", { key: profile.profile_id, value: profile.profile_id }, profile.display_name || profile.profile_id); }))),
          h("button", { className: "primary-button", disabled: !runDraft.workflow_id || !runDraft.source_job_id || !runDraft.entity_id || !runDraft.model_profile_id, onClick: createRun }, "创建并调度"))),
      h("div", { className: "agent-runs-layout workflow-runs-layout" },
        h("section", { className: "surface agent-run-list" }, h("div", { className: "section-title" }, h("h3", null, "运行记录"), h("span", null, runs.length + " 个")), runs.map(function (run) { return h("button", { key: run.workflow_run_id, className: "agent-run-row " + (selected && selected.workflow_run_id === run.workflow_run_id ? "active" : ""), onClick: function () { props.onSelect(run.workflow_run_id); } }, h("div", null, h("b", null, run.entity_id), h("small", null, shortHash(run.workflow_run_id) + " · " + timeText(run.created_at))), h("span", { className: "status-chip " + run.status }, run.status), h("em", null, run.used_tool_calls + "/" + run.max_tool_calls)); })),
        h("section", { className: "surface agent-run-detail workflow-run-detail" }, selected ? h(React.Fragment, null,
          h("div", { className: "section-title" }, h("h3", null, "DAG 运行图"), h("span", { className: "status-chip " + selected.status }, selected.status)),
          h("div", { className: "workflow-run-actions" }, actions(selected.status).map(function (action) { return h("button", { key: action, className: action === "cancel" ? "danger-button" : "secondary-button", onClick: function () { props.onAction(selected.workflow_run_id, action); } }, actionLabel(action)); }), selected.status === "awaiting_human" && h("button", { className: "primary-button", onClick: props.onReview }, "去人工审批")),
          h("div", { className: "workflow-dag" }, (selected.nodes || []).map(function (node) { const role = roles.find(function (item) { return item.role_id === node.role_id; }) || {}; return h("article", { key: node.node_id, className: "workflow-node " + node.status }, h("span", { className: "dependency-line" }, node.dependencies.length ? node.dependencies.join(" + ") + " →" : "起点"), h("b", null, role.name || node.role_id), h("small", null, node.node_id + " · attempt " + node.attempt), h("em", null, node.status), node.status === "failed" && h("button", { className: "text-button", onClick: function () { props.onRetryNode(selected.workflow_run_id, node.node_id); } }, "重试节点")); })),
          selected.error_code && h("div", { className: "agent-run-failure" }, h("b", null, selected.error_code), h("span", null, selected.error_summary)),
          h("h4", null, "Artifact 与 Gate"), h("div", { className: "agent-artifact-list workflow-artifacts" }, (selected.artifacts || []).length ? selected.artifacts.map(function (artifact) { return h("article", { key: artifact.artifact_id }, h("div", null, h("b", null, artifact.artifact_type), h("small", null, artifact.role_id + " · " + artifact.node_id)), h("span", null, artifact.payload && artifact.payload.candidate_id ? "Candidate " + shortHash(artifact.payload.candidate_id) : "受控节点结果")); }) : h("div", { className: "empty-state compact" }, "暂无受控产物；Candidate 仍需经过人工 Gate")),
          h("h4", null, "节点日志与事件"), h("div", { className: "agent-event-list" }, (selected.events || []).slice().reverse().map(function (event) { return h("div", { key: event.event_id }, h("span", null, "#" + event.sequence), h("b", null, event.type), h("small", null, event.node_id || "Run")); }))) : h("div", { className: "empty-state" }, "选择工作流 Run 查看 DAG、节点日志和预算"))),
      h("p", { className: "workflow-security-note" }, "安全边界：工作流只编排注册角色；所有 Candidate 经过确定性 Evaluator 和人工 Gate 后才可能成为批准规则。"));
  }
  ...
  function App() {
    ...
    const [agentWorkflowData, setAgentWorkflowData] = useState({ workflows: [], runs: [], selected: null, loading: false, failure: "" });
    ...
    function changeView(next) {
      ...
      if (next === "agentWorkflows") loadAgentWorkflows().catch(function (reason) { setError(reason.message); });
    }
    ...
    async function loadAgentWorkflows(selectedId) {
      setAgentWorkflowData(function (current) { return Object.assign({}, current, { loading: true, failure: "" }); });
      try {
        const values = await Promise.all([api.agentWorkflows(), api.agentWorkflowRuns()]), runs = values[1].items || [];
        const target = selectedId || (agentWorkflowData.selected && agentWorkflowData.selected.workflow_run_id) || (runs[0] && runs[0].workflow_run_id);
        const detail = target ? await api.agentWorkflowRun(target) : null;
        setAgentWorkflowData({ workflows: values[0].items || [], runs: runs, selected: detail, loading: false, failure: "" });
      } catch (reason) { setAgentWorkflowData({ workflows: [], runs: [], selected: null, loading: false, failure: reason.message }); throw reason; }
    }
    async function changeAgentWorkflow(runId, action) { const result = await api.agentWorkflowAction(runId, action); await loadAgentWorkflows(action === "retry" ? result.workflow_run_id : runId); return result; }
    ...
    async function loadRuntime() {
      setRuntimeData(function (current) { return Object.assign({}, current, { loading: true, failure: "", failureCode: "" }); });
      try {
        const values = await Promise.all([api.runtimeIdentity(), api.runtimeHealth(), api.runtimeReadiness(), api.runtimeTasks(), api.runtimeStorage(), api.runtimeRetention(), api.runtimeAudit()]);
        const next = { identity: values[0].identity || {}, health: values[1] || {}, readiness: values[2] || {}, tasks: values[3] || { items: [] }, storage: values[4] || {}, retention: values[5] || {}, audits: values[6] || { items: [] }, loading: false, failure: "", failureCode: "" };
        setRuntimeData(next);
        return next;
      } catch (reason) {
        setRuntimeData(function (current) { return Object.assign({}, current, { loading: false, failure: reason.message || "未知错误", failureCode: reason.code || "" }); });
        throw reason;
      }
    }
    ...
    const selectedRepresentative = selectedReview && selectedReview.representative || null;
    const topbarModel = view === "review" ? (selectedRepresentative && selectedRepresentative.model || "—") : model;
    ...
    return h("div", { className: "app-shell" + (sidebarCollapsed ? " sidebar-collapsed" : "") + (mobileMenuOpen ? " mobile-menu-open" : "") },
      h("header", { className: "topbar" }, h("div", { className: "topbar-left" }, h("button", { className: "sidebar-toggle", type: "button", "aria-label": narrowSidebar ? (mobileMenuOpen ? "关闭菜单" : "打开菜单") : (sidebarCollapsed ? "展开菜单" : "折叠菜单"), "aria-controls": "primary-navigation", "aria-expanded": narrowSidebar ? mobileMenuOpen : !sidebarCollapsed, onClick: toggleSidebar }, h("i"), h("i"), h("i")), h("div", { className: "brand" }, h("img", { className: "brand-logo", src: "/assets/logrisk-app-icon-orange-v2.png", alt: "LOGRISK" }), h("div", null, h("b", null, "LOGRISK"), h("span", null, "Feature review console")))), h("div", { className: "topbar-actions" }, h("a", { className: "help-button", href: apiUrl("/help"), target: "_blank", rel: "noreferrer" }, "? 帮助"), h("div", { className: "system-status" }, h("span", { className: ollama.online ? "online" : "offline" }, "● Ollama " + (ollama.online ? "在线" : "离线")), h("span", null, topbarModel), h("button", { className: "prompt-pill", onClick: function () { changeView("prompts"); } }, "Prompt " + (harness.current_prompt_id || promptId)), h("span", { className: harness.trace_enabled ? "trace-on" : "trace-off" }, "● Trace " + (harness.trace_enabled ? "ON" : "OFF"))))),
      ...
      h("main", null,
        ...
        view === "observability" && h(AIObservabilityPage, { summary: obsSummary, progress: obsProgress, events: obsEvents, onRefresh: function () { loadObservability(obsProgress && obsProgress.job_id).catch(function (reason) { setError(reason.message); }); }, onOpenTrace: openTrace, onOpenPrompt: openPrompt, onModels: function () { changeView("modelProfiles"); }, onReview: function () { changeView("review"); }, onRules: function () { changeView("rules"); }, onNewAnalysis: function () { changeView("overview"); } }),
