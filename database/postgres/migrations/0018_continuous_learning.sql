...
ALTER TABLE drain_datasets ADD COLUMN approved_at TIMESTAMPTZ;
...
SET dataset_family_id = COALESCE(dataset_family_id, dataset_id),
    ...
    record_count = COALESCE(
        record_count,
        CASE
            WHEN jsonb_typeof(dataset_json->'records') = 'array' THEN jsonb_array_length(dataset_json->'records')
            ELSE 0
        END
    ),
