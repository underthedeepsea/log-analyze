...
- 审批与候选登记复用数据库事务和身份锁，规则、候选、审批组与决定原子写入；审批支持版本检查和请求幂等重放，移除可能覆盖较新决定的无版本补偿回退。可选审批观测在事务成功后记录，失败事务不留下成功轨迹。SQLite/PostgreSQL 新增 `0023_approval_transactions.sql`。
- 修复 V1 批准规则复用引用未定义上下文，保留原特征类型、模板、组件和锚点严格匹配范围。
- 审批界面保留失败草稿，明确区分保存中、已保存和冲突；相同请求重试沿用幂等键，冲突展示服务器最新决定。新候选和已批准规则健康复审分别标识，查询保持只读。 基于最新决定编辑时禁用反向审批，保持后台终态保护；冲突说明和操作按钮换行展示，避免窄列挤压。
- 修复真实 Agent Evaluator 诊断与工具脱敏契约不兼容；校验凭据绑定候选、证据与 Evaluator 版本，只读专家产物经受控依赖传递，规则检索覆盖后续分页。
- 统一 Django 与独立 Dashboard 的 Kafka 布尔配置和实例适配器隔离，提供依赖、连接检查与活动任务状态；保持默认关闭和有限批次读取。

## 1.38.0 - 2026-09-11

### Added

- 人工审批支持前端后台提交队列，最多同时保存两组；提交后立即显示绿色状态并保留条目、选择和审批面板，可继续处理下一组。明确区分保存中、已保存与失败，失败保留草稿并支持重试，存在未确认提交时关闭页面会提示。
- 审批队列按需加载，每次最多显示新增 100 组；提供按稳定 Review Key 翻页，避免审批移除前页记录后跳过后续待审批组。

### Fixed

- 将审批反馈固定在编辑面板内部，修复驳回提示挤占三栏布局、导致审批窗口换行的问题；避免切换候选时沿用上一条的模板生成草稿；浏览器前进/后退同步当前审批对象，离开未保存草稿时提示。
- SQLite/PostgreSQL 候选查询按任务批量读取小型模型元数据，不再为每个候选传输和解析整个任务快照。
- 同身份审批改为在一个事务内批量更新候选、分组和追加审计事件；按证据重算身份、加锁后二次检查状态，保留并发保护和人工审批门禁。人工审批不再反复重写整个任务及全部候选。
- 修复 PostgreSQL 无参数迁移 SQL 的 JSON `?` 运算符被误转换为参数占位符，确保历史规则迁移可在实际 PostgreSQL 上执行。
- 新增审批队列复合索引，以及千条队列、批量审批、事务回滚、并发重复提交和后台保存状态的回归覆盖。

## 1.37.3 - 2026-09-11

### Fixed

- 修复带历史物理审批键的现代候选被误判为 V1、无法命中有效 V2 规则的问题；继续按重新派生的证据身份匹配，缺失与冲突证据仍受原门禁约束。
- 为 0004 迁移误标为 V2 的旧批准规则提供显式修复：必须匹配未变更的原始系统迁移快照，追加治理版本和审计事件，恢复原 V1 格式及严格匹配范围；不将任意损坏 V2 规则自动降级。
- 补充停止容器超时、内存限制配置无效、资源更新写入忙、重复移除、沙箱停止失败、静态清单目录缺失、preStop 失败与 Cgroup 设备缺失的确定性分类。概括性镜像拉取失败仍保留复核，不推断具体原因。
- 提供默认只预览的离线待审批重分类函数；在同一事务中更新候选、审批组、实体引用、任务快照和修复事件，保留原候选 ID、Trace 与模板来源。已结束、人工编辑、有反馈及校验失败的候选不自动改写，运行中任务阻止修复。
- 增加迁移来源校验、人工审批保护、拆分计数、重复执行、事务回滚及重启后规则复用的回归覆盖。

## 1.37.2 - 2026-09-09

### Fixed

- 补充 `network calico/k8s-pod-network no enough ips` 的 CNI 上下文，避免同类 IP 耗尽证据落入独立未解析审批。
- 将明确的 `RecentStats unable to find data in memory cache` 识别为缓存未命中，消除同一诊断同时命中统计失败造成的伪冲突；同模板内其他独立统计失败仍保持冲突门禁。
- 补充 `error cleaning subPath mounts` 失败模式，使长路径和 `/proc/mounts` 内容不一致变体正确拆分并归入卷子路径清理失败。
- 未解析候选计算审批身份时排除已生成的 `logrisk.*` 兜底问题码，防止保存和重复读取把旧哈希再次作为证据，导致审批键持续漂移；仍保留严格模板集匹配与冲突门禁。
- 新增当前待审批模板变体、负例与五组缓存证据收敛回归测试。修复作用于新生成候选，保留当前审批状态与原始候选记录。

## 1.37.1 - 2026-09-09

### Fixed

- 按所选模板分别生成已识别语义候选与未解析证据候选，未知或不可分割的冲突模板保持严格 template-set 审批，不再使其他明确语义整体回退。各候选独立计算所选日志次数，保留全部模板且不重复分配。
- 使用确定性类别标题、摘要与类型；未知证据统一标记待复核，避免模型命名漂移拆散相同证据的审批，以及将混合证据推断为节点资源不足。缺失模板、无效组件与语义回滚仍保留原门禁。
- 补充完整 WorkloadEndpoint 资源名、长文件系统统计路径、checkpoint 资源分配记录缺失、镜像请求 EOF 与 unknown artifact 模板变体。
- 审批界面区分未记录、校验失败、结构通过但语义待复核与语义证据完整，日志命中次数不再混称事件出现次数。同步独立 Dashboard 与 Django 静态资源。
- 本次修复作用于新生成候选，不自动改写历史候选、审批状态或已批准规则。

## 1.37.0 - 2026-09-06

### Fixed

- 收紧所选脱敏模板的共享语义门禁：任一已选模板未知、缺失或冲突时保留候选和诊断，但只能进入严格 template-set 审批与规则复用路径。
- 在语义拆分和源事实附加后为每个最终候选分别记录评估结果；证据不完整的候选明确标记为待人工复核。
- 加强候选输出校验：所选模板组件必须归属于该候选的所选证据，同一次响应不得重复分配模板 Hash，标题、摘要和选择理由统一拦截禁止的 RCA/处置断言。
- 支持 `WorkloadEndpoint <*> not found` 与 `does not exist` 的受控匹配变体，并将安全语义审批组标题标准化为注册的问题类别展示。
- 禁止不完整输入通过历史语义规则或恢复路径复用（包括未标注 `match_mode` 的 V1 规则）；非语义候选仅可精确匹配完整的 template-set 规则。
- 审批协调、批量驳回与已批准组恢复只按重新派生的逻辑身份授权；历史物理审批键仅保留为审计定位。V1 特征与实体复用统一要求特征类型、完整模板集、组件和锚点精确一致，并支持从实体补全候选明确选中的 bodyless 模板证据。
- V1 精确身份比较在归一化前拒绝规则、特征或实体缺失的 `feature_type`，防止双方缺失时因 `unknown_feature` 误授权。
- 现代非语义审批组使用全部所选模板标识重算逻辑键，同时保留历史物理键供审计展示。
- 使未显式指定的提取 Prompt 在实际解析点遵从所选模型 Profile；新建特征任务持久化并在运行、重试和恢复时复用 Prompt ID、哈希、内容、路径与版本快照，历史无快照任务保持原行为。
- 候选持久化保留所选证据缺失与无法解析的 ID 诊断，避免只在审批队列读取时临时重算。
- AI 缓存键升级为包含无敏感信息的有效生成参数与输出 Schema 摘要，避免调参误用旧模型输出。
- 长脱敏模板使用带明确省略标记的确定性首尾摘录，并记录模型可见片段的逐模板截断信息；Evidence 总字符预算覆盖实体、模板元数据和截断元数据，无法容纳最小 envelope 时在模型调用前失败。
- 修复只读审批重分类工具的中英文/BOM CSV 兼容、证据不足标记和旧物理/逻辑身份对比；CSV 与直接重分类入口共用候选 ID/状态归一化，所有中英文候选状态都优先于审批组/队列回退列，并一致拒绝冲突的候选 ID。历史批次仅生成审计报告，不修改原 CSV 或审批状态。
- 将所选模板的来源优先级和标识筛选收敛为 resolver 共享 API，避免 CSV 证据门禁与审批解析器发生漂移。

### Added

- 注册保持非默认状态的 v4 原子证据特征提取 Prompt；Qwen3.5 4B MLX Profile 继续默认使用 v3。

## 1.36.4 - 2026-09-04

### Fixed

- 新增基于安全语义解析的混合特征确定性分区与完整问题展示注册表，按选定脱敏模板顺序生成单一异常语义子特征，并保留不安全输入的原候选回退。
- 修复复合容器统计与容器不存在模板的确定性语义优先级：保留 `container_stats_failure` 主问题，并将 `container_not_found` 作为子类型；新增单模板解析公开接口，同时保持通用 wrapper 的严格安全门禁。
- 补充单模板解析四种结果分支的直接参数化回归覆盖，并锁定复合统计与独立容器不存在场景的版本化匹配规则。
- 模型特征在源事实挂接与 Candidate 持久化前执行确定性分区；未解析的混合特征保持原候选回退。
- 修复 Review Workbench 改为进入页面首次加载、手动刷新和审批后刷新，分别展示 Review Group 与候选数量，并移除 Review 轮询和可见性自动刷新。
- 修复持久化候选在审批队列重解析时按最新脱敏模板证据更新语义，并保持 Review Group 与审批指标一致。

- 修复容器统计获取失败因日志中包含 `unknown/no such container` 而被错误合并到 `container_not_found` semantic Group 的问题；统计操作保持主审批语义，容器不存在作为 subtype 保留。
- 对模型选中、且可被高置信 deterministic resolver 安全分区的多语义模板，在 Candidate 持久化前拆成独立单语义 Candidate；无法安全分区的输入仍保持 strict fallback。
- 人工审批页移除 3 秒自动轮询和标签页恢复自动刷新，改为首次加载、手动刷新以及审批动作后刷新。
- 人工审批页同时显示待审批 Review Group 数和底层 Candidate 数，避免将 Candidate 总数误解为审批次数。
